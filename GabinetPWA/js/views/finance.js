...
const FinanceViews = (() => {
  ...
  function revenueThisMonth() {
    return paidSessionsThisMonth().reduce(function(sum,s){ return sum + (s.price||0); }, 0);
  }
  ...
  function totalDebt() {
    return unpaidRequiredSessions().reduce(function(sum,s){ return sum + (s.price||0); }, 0);
  }
  ...
  function revenueThisYear() {
    ...
    return getSessions().filter(function(s) {
      const d = new Date(s.date + 'T00:00:00');
      return d.getFullYear() === ym.year && s.isPaid;
    }).reduce(function(sum,s){ return sum + (s.price||0); }, 0);
  }
  ...
  function buildUnpaidSessionOptions(patientId, selectedIds) {
    ...
    return sessions.map(function(s) {
      ...
      const label = formatDateLong(s.date) + ' ' + (s.startTime||'')+' \u2013 ' + formatCurrency(s.price);
    ...
    (p.sessionIds || []).forEach(function(sid) {
      ...
      paidSessions +=
        '<div class="fin-detail-session-row">' +
          ...
          '<span class="fin-ds-time">' + (s.startTime||'')+'</span>' +
          '<span class="fin-ds-amount">' + formatCurrency(s.price) + '</span>' +
        '</div>';
    });
