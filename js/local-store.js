...
const LocalStore = {
  ...
  getSyncStatusSummary() {
    ...
    if (!state.supported) {
      return {
        status: 'Lokalny snapshot niedostepny',
        note: 'Ta przegladarka nie udostepnia bezpiecznego magazynu offline.',
        actionLabel: connected ? null : 'Polacz z Google',
    ...
    if (!state.snapshotAvailable) {
      return {
        ...
        note: 'Lokalna kopia pojawi sie po pierwszym udanym wczytaniu albo zapisie danych.',
        actionLabel: connected ? null : 'Polacz z Google',
        bannerVisible: false,
      };
    }
    ...
    if (!connected) {
      return {
        status: state.hasPendingSync ? 'Lokalne zmiany czekaja na synchronizacje' : 'Dane lokalne dostepne',
        note: state.hasPendingSync
          ? 'Pracujesz na danych z tego urzadzenia. Polacz z Google, aby wyslac zmiany na Drive.'
          : 'Mozesz pracowac na lokalnej kopii danych. Polacz z Google, aby zsynchronizowac najnowszy stan.',
        actionLabel: 'Polacz z Google',
        bannerVisible: true,
      };
    }
    ...
    if (state.hasPendingSync) {
      return {
        status: online ? 'Synchronizacja lokalnych zmian' : 'Zmiany czekaja na internet',
        note: online
          ? 'Lokalna kopia zostala zaktualizowana i czeka na zapis do Google Drive.'
          : 'Lokalna kopia jest bezpieczna. Zapis do Google Drive wroci, gdy internet bedzie znowu dostepny.',
        actionLabel: null,
        bannerVisible: true,
      };
    }
