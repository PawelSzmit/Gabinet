...
const CalendarViews = {
  ...
  renderFocusPanel() {
    ...
    const dayKey = this._dateKey(focusDate);
    const todaySessions = getSessionsByDate(dayKey)
      .sort((a, b) => new Date(a.date) - new Date(b.date));
    const completedToday = todaySessions.filter((session) => session.status === 'completed').length;
    const unpaidSessions = AppState.sessions.filter((session) => {
    ...
    const blockedToday = this._isDateBlocked(focusDate);
    const debtPatients = new Set(unpaidSessions.map((session) => session.patientId)).size;
    const dayLabel = formatDateWithWeekday(focusDate);
    const nextSessionHtml = nextSession
      ? (
          '<div class="cal-focus-next">'
          + '<span class="cal-focus-next__eyebrow">Najbliższa sesja</span>'
          + '<strong>' + this._escapeHtml(nextPatient ? (nextPatient.pseudonym || nextPatient.firstName) : 'Pacjent') + '</strong>'
          + '<span>' + formatDateTimeWithWeekday(new Date(nextSession.date)) + '</span>'
          + '</div>'
        )
      : (
          '<div class="cal-focus-next cal-focus-next--empty">'
          + '<span class="cal-focus-next__eyebrow">Najbliższa sesja</span>'
          + '<strong>Spokojniejszy moment w kalendarzu</strong>'
          + '<span>Brak zaplanowanych sesji w kolejce.</span>'
          + '</div>'
        );
  ...
  bindEvents() {
    ...
    safeClick('today-btn-add-session', this.showAddSessionModal);
    safeClick('today-btn-add-payment', () => Router.navigate('finance'));
    safeClick('today-btn-open-patients', () => Router.navigate('patients'));
    const grid = document.getElementById('cal-grid');
    if (grid) {
  ...
  _injectStyles() {
    ...
    const rules = [
      ...
      '.cal-btn-add:hover{background:linear-gradient(135deg,var(--blue,#49664f),#6b8b72)}',
      '.cal-segment{display:flex;background:rgba(255,255,255,.48);border-radius:999px;padding:4px;gap:4px;border:1px solid var(--border,rgba(73,102,79,.12))}',
      ...
      '.cal-week-scroll{flex:1;min-height:0;overflow:auto;}',
      '.cal-week-header{display:grid;grid-template-columns:50px repeat(7,minmax(110px,1fr));background:#fff;border-bottom:1px solid #e0e0e5;position:sticky;top:0;z-index:5;min-width:calc(50px + 7*110px)}',
      ...
      '.cal-week-body{display:grid;grid-template-columns:50px repeat(7,minmax(110px,1fr));min-width:calc(50px + 7*110px)}',
      '.cal-week-time-col{border-right:1px solid #e0e0e5}',
      '.cal-week-time-label{font-size:.68rem;color:#8e8e93;padding:4px 4px 0;height:40px;border-bottom:1px solid #f2f2f7;display:flex;align-items:flex-start;box-sizing:border-box}',
      '.cal-week-day-col{border-right:1px solid #f2f2f7}.cal-week-day-col.today{background:#f0f7ff}',
      '.cal-week-slot{height:40px;border-bottom:1px solid #f2f2f7;padding:2px;overflow:hidden;box-sizing:border-box}',
      ...
      '.cal-sessions-list-header{display:none}',
      ...
      '.cal-modal-overlay{position:fixed;inset:0;background:var(--surface-overlay,rgba(0,0,0,.5));z-index:9000;display:flex;align-items:center;justify-content:center;padding:16px}',
      '.cal-modal-sheet{background:var(--surface,#fff);border-radius:16px;width:100%;max-width:600px;max-height:88vh;overflow-y:auto;animation:calFadeIn .22s ease}',
      '.cal-modal-sheet-large{max-height:92vh}',
      '@keyframes calFadeIn{from{opacity:0;transform:scale(.97)}to{opacity:1;transform:scale(1)}}',
      '.cal-modal-header{display:flex;align-items:center;justify-content:space-between;padding:14px 16px;background:var(--surface,#fff);border-bottom:1px solid var(--border,#e0e0e5);position:sticky;top:0;z-index:1}',
      '.cal-modal-title{font-size:.95rem;font-weight:700;color:var(--text,#1c1c1e);margin:0}',
      ...
      '.cal-form-label{font-size:.75rem;font-weight:600;color:var(--text-secondary,#8e8e93);text-transform:uppercase;letter-spacing:.03em}',
      '.cal-form-control{border:1px solid var(--border,#d1d1d6);border-radius:10px;padding:10px 12px;font-size:.9rem;background:var(--surface-raised,#f2f2f7);color:var(--text,#1c1c1e);outline:none;width:100%;box-sizing:border-box;-webkit-appearance:none;appearance:none;font-family:inherit}',
      ...
      '.cal-detail-section{background:var(--surface-raised,#f7f7f7);border-radius:12px;padding:12px 16px;margin-bottom:12px}',
      ...
      '.cal-detail-row{display:flex;align-items:flex-start;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border,#f2f2f7);gap:12px}',
      '.cal-detail-row:last-child{border-bottom:none}',
      '.cal-detail-label{font-size:.85rem;color:#8e8e93;flex-shrink:0}',
      '.cal-detail-value{font-size:.85rem;color:var(--text,#1c1c1e);font-weight:500;text-align:right}',
      '.cal-detail-reschedule .cal-detail-label{color:#FF9500}',
      '.cal-detail-patient{text-align:center;padding:18px 16px}',
      '.cal-detail-patient-name{font-size:1.1rem;font-weight:700;color:var(--text,#1c1c1e)}',
      ...
      '@media (prefers-color-scheme: dark){.cal-wrapper{background:transparent}.cal-focus-panel,.cal-toolbar,.cal-day-sessions{background:color-mix(in srgb,var(--surface-raised,#223128) 88%, transparent)}.cal-focus-next,.cal-focus-stat,.cal-add-menu{background:rgba(255,255,255,.04)}.cal-focus-title,.cal-title,.cal-row-name,.cal-daily-event-name,.cal-daily-event-time,.cal-sessions-list-title,.cal-detail-value,.cal-detail-notes-text,.cal-detail-patient-name{color:var(--text,#f4ede4)}.cal-focus-text,.cal-focus-next span,.cal-focus-stat span,.cal-focus-stat small,.cal-add-menu-item,.cal-detail-label,.cal-detail-muted,.cal-header-cell,.cal-week-header-cell,.cal-week-event-name,.cal-row-time{color:var(--text-secondary,rgba(244,237,228,.72))}.cal-segment{background:rgba(255,255,255,.05)}.cal-seg-btn.active{background:rgba(255,255,255,.08);color:var(--blue,#dcc29d)}.cal-btn,.cal-form-control,.cal-detail-section,.cal-modal-header{background:rgba(255,255,255,.04);color:var(--text,#f4ede4);border-color:var(--border,rgba(220,194,157,.12))}.cal-week-body,.cal-week-header,.cal-daily-header,.cal-grid-headers,.cal-grid,.cal-cell,.cal-cell-other,.cal-week-slot,.cal-daily-row,.cal-session-row{border-color:var(--separator,rgba(220,194,157,.08))}.cal-cell{background:#213027}.cal-cell-other{background:#1b271f}.cal-cell-selected{background:rgba(220,194,157,.08)!important}.cal-session-row:hover{background:rgba(255,255,255,.04)}' +
        '.cal-day-num{color:rgba(244,237,228,.82)!important}' +
        '.cal-cell-weekend .cal-day-num{color:rgba(244,237,228,.42)!important}' +
        '.cal-cell-today .cal-day-num{background:#4a8cca;color:#fff!important}' +
        '.cal-cell-blocked{background:rgba(255,59,48,.12)!important}' +
        '.cal-week-header{background:#1e2d24}' +
        '.cal-week-header-cell.selected{background:rgba(220,194,157,.08)}' +
        '.cal-week-day-col.today{background:rgba(220,194,157,.05)}' +
        ...
        '.cal-week-scroll::-webkit-scrollbar{width:6px;height:6px}.cal-week-scroll::-webkit-scrollbar-track{background:transparent}.cal-week-scroll::-webkit-scrollbar-thumb{background:rgba(220,194,157,.25);border-radius:3px}' +
        '}',
